...
def mkTempPath(base_dir='C:\\Users\\Administrator\\Desktop\\buildaab\\output', prefix='', suffix=''):
...
def mkTmpCompiledResourcesPath(base_path):
    print("mkTmpCompiledResourcesPath ="+base_path)
    return mkTempPath(base_path)
...
#随机目录
def generate_directory_path(base_dir, *args):
    """
    Generate a directory path based on the given parameters.

    Args:
        base_dir (str): The base directory where the new directory will be created.
        args (str): Additional directory names to be appended.

    Returns:
        str: The generated directory path.
    """
    directory_path = os.path.join(base_dir, *args)
