...
def loadEnv():
    ...
    #添加Windows路径
    WinPath = "C:\\Users\\Administrator\\Desktop\\buildaab\\dist"
    with open(os.path.join(WinPath, "project.config")) as f:
...
def compiledRes(resPath, compiledPath):
    if os.path.exists(compiledPath):
        execCmd("rmdir /s /q {}".format(compiledPath))
...
def processAAB(apkPath, signName=None, signPwd=f"{defaultPwd}", outDir="out",
    ...
    print("public_xml_path ="+public_xml_path)
    out_path_public_xml = os.path.join(out_path, "public_old.xml")
    print("out_path_public_xml ="+out_path_public_xml)
    ...
    print("apktool.yml ="+os.path.join(apk_source,"apktool.yml"))
    ...
    if stableIds:
        ...
        print("versioz znInfo ="+versionInfo)
    
        print("android_manifest_path ="+android_manifest_path)
        print("output_apk ="+output_apk)
        print("stableIds=public_txt_path ="+str(stableIds=public_txt_path))
    ...
    print("resmap_new_path ="+resmap_new_path)
...
def aapt2CompileRes(apkPath, output):
    ...
    compiled_resources_dir = zip.create_zip_with_random_files(tmp_output)
...
if __name__ == '__main__':
    ...
    if argv1.lower().startswith("--compile-res"):
        if len(sys.argv) > 2:
            ...
            tmp_compiled_path = zip.create_zip_with_random_files(tmp_out_path)
    ...
    if argv1.lower().startswith("--link-res"):
        if len(sys.argv) > 2:
            ...
            temp_compiled_resources = zip.create_zip_with_random_files(tmp_out_path)
